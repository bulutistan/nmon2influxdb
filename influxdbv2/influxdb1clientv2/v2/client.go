...
import (
	...
	"errors"
	"fmt"
	"github.com/adejoux/nmon2influxdb/influxdbv2/influxdb1clientv2/models"
	"io"
	"io/ioutil"
	"mime"
	"net/http"
...
func NewHTTPClient(conf HTTPConfig) (Client, error) {
	...
	u2, err := url.Parse(conf.Addr + "/api/v2")
	if err != nil {
		return nil, err
	} else if u.Scheme != "http" && u.Scheme != "https" {
		m := fmt.Sprintf("Unsupported protocol scheme: %s, your address"+
...
func (c *client) Ping(timeout time.Duration) (time.Duration, string, error) {
	...
	u := c.urlV2
	u.Path = path.Join(u.Path, "signin")
	...
	c.session = resp.Header.Get("Set-Cookie")
...
type BatchPoints interface {
	...
	// Organization returns the currently set database of this Batch.
	Organization() string

	// SetOrganization sets the database of this Batch.
...
func (bp *batchpoints) Organization() string {
	return bp.organization
}
...
func (bp *batchpoints) SetOrganization(org string) {
	bp.organization = org
}
...
func (c *client) Write(bp BatchPoints) error {
	...
	//if c.username != "" {
	//	//req.SetBasicAuth(c.username, c.password)
	//	req.Header.Set("Authorization", TokenAuth(c.username, c.password))
	//}
	...
	params := req.URL.Query()
	params.Set("bucket", bp.Database())
	params.Set("org", bp.Organization())
...
func TokenAuth(username, password string) string {
	auth := username + ":" + password
	//return auth
	return fmt.Sprintf("Token %s", base64.StdEncoding.EncodeToString([]byte(auth)))
}
